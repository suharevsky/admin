...
import {MessageService} from '../../../../inbox/_services/message.service';
...
export class UpdateCustomersStatusModalComponent implements OnInit, OnDestroy {
  ...
  constructor(private messageService: MessageService, public modal: NgbActiveModal) { }
  ...
  loadCustomers() {
    const sb = this.messageService.items$.pipe(
      first()
  ...
  updateCustomersStatus() {
    this.isLoading = true;
    const sb = this.messageService.updateStatusForItems(this.ids, +this.status).pipe(
