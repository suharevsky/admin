...
export class EditCustomerModalComponent implements OnInit, OnDestroy {
  ...
  loadCustomer() {
    if (!this.id) {
      ...
      const sb = this.messageService.getItemById(this.id).pipe(
        first(),
  ...
  edit() {
    const sbUpdate = this.messageService.update(this.customer).pipe(
      tap(() => {
        this.modal.close();
      }),
      catchError((errorMessage) => {
  ...
  create() {
    const sbCreate = this.messageService.create(this.customer).pipe(
      tap(() => {
        this.modal.close();
      }),
      catchError((errorMessage) => {
